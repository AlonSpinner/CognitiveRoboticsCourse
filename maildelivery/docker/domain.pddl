(define (domain maildelivery-domain)
 ...
 (:predicates (robot_at ?r - robot ?l - location) (is_connected ?l_from - location ?l_to - location) (is_free ?l - location) (robot_has_package ?p - package ?r - robot) (location_has_package ?p - package ?l - location) (robot_can_hold_package ?r - robot) (not_targeted ?l_to - location))
 ...
 (:durative-action move
  ...
  :condition (and (at start (is_connected ?l_from ?l_to))(at start (not_targeted ?l_to))(at start (robot_at ?r ?l_from))(at start (<= (* 2 (distance ?l_from ?l_to)) (charge ?r)))(at end (is_free ?l_to)))
  :effect (and (at start (not (robot_at ?r ?l_from))) (at start (is_free ?l_from)) (at start (not (not_targeted ?l_to))) (at end (robot_at ?r ?l_to)) (at end (not (is_free ?l_to))) (at end (not_targeted ?l_to))))
 (:action pickup
